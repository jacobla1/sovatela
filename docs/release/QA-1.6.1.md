...
A fourth round at **`c7164cf`** took six findings from the September 2026
review — F-06, F-07, F-09, F-14, F-16 and F-18. All four workflows are green
there, and `npm test` is **473**.
...
### What the fourth round's CI caught

Three failures across two runs, none of them reachable from this machine, and
each worth recording because the pattern is now consistent: everything that
broke was in a build or on a platform that is not run here.

| Failure | What it was |
| --- | --- |
| `vetted_external_url("https:///nowhere")` asserted to fail | The assertion was wrong, not the code. The URL spec tolerates extra slashes after a special scheme, so that string parses to the host `nowhere` and opening it is correct |
| `Option::is_none_or` | Stable since 1.82; this crate's MSRV is 1.77.2. `pricing.rs` had already hit this and left the note — reading the neighbouring guard would have prevented it |
| `.err().expect()` on a Result whose Ok type is Debug | Clippy wants `expect_err`. The identical line in `secrets_parsing` is correct and is not flagged: `Secrets` deliberately has no `Debug`, so `expect_err` there would not compile, and the lint checks for Debug before firing |

Everything else in the batch — the streaming caps, the no-follow workspace
opens, the scheme allowlist, the credential-store parser — compiled and passed
on all three platforms first time.

**Not verified here:** the release feed is generated by `deploy/web/build.mjs`,
which needs the published artifacts to run, so it has been unit-tested against
the real release notes but never built. Its first real run is the next release.
...
| The release feed as actually served | **not built** — `build.mjs` needs the published artifacts; unit-tested against the real notes only |
| The workspace symlink guard on Windows | `symlink_metadata` is a check before the open, not a guarantee like `O_NOFOLLOW`; the Unix tests do not cover Windows |
| An ancestor directory swapped mid-operation | still followed — the remaining half of F-09, recorded in TECHNICAL-SPEC § 7.2 |
