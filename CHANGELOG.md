...
template of yours if you supply one — and the release also carries a fix for
something that has been broken in the three releases before it.

### New
- **Documents are a first-class artifact type.** Ask for a report, a sheet or a
  deck and it arrives as an artifact you can preview and save, or have written
  into your workspace folder. The model writes Markdown or tabular text and the
  app builds the file; it never emits OOXML. A new `ooxml` module writes all
  three formats, and every file it produces is checked before anyone is handed
  it — a part cannot be present and undeclared, declared and absent, or point
  at something that is not there.
- **The preview is generated by the writer.** It used to be `marked` in the
  renderer while the file was built by a deliberately small parser in Rust;
  measured across twenty-four constructs they disagreed on sixteen. Both now
  read the same answer, so what you check before sending is what you send.
- **Bring your own template.** Point at any `.docx`, `.dotx`, `.pptx` or
  `.potx` in *Settings → Document templates* and generated files come out in
  its design — fonts, colours, headings, page size, and any header or footer.
  Its text, its slides and its pictures stay behind, so last quarter's report
  works as a template exactly as it is.

### Security
- **A template could make every document you generated phone home.** Templates
  are refused when they carry macros, link to something outside themselves, or
  hold a field that fetches when the document is opened — an `INCLUDEPICTURE`
  in a header reaches out on the *recipient's* machine, not yours. That check
  did not cover the one part copied out of a template without being checked:
  the section properties in `word/document.xml`. The same field placed there
  was accepted, written into every generated document, and fetched by Word when
  the file was opened. Found by pointing the URL at a listener and watching it
  arrive; the section now goes through the same check as everything else.
- **A template's pictures no longer ride along.** The media folder was copied
  whole, filtered only by whether a file was a picture rather than an
  executable — a check that cannot tell a logo the slide master draws from a
  photograph that was on slide 7. So an ordinary filled-in report used as a
  template carried its content images into every document generated from it:
  unreferenced, invisible to a reader, and inside a file you send to someone
  else. Only pictures a copied part actually points at are taken now.
- Every job in `ci.yml` is guarded against running on the public mirror. It was
  the one job in any workflow without that line.

### Fixed
- **Artifacts have not run their own JavaScript since 1.5.3.** The window
  stopped allowing inline script in that release — correctly; the built page
  has none. But an artifact frame was built with `srcdoc`, and a `srcdoc`
  document inherits the window's policy and cannot widen it, so every artifact
  inherited a rule that refused its scripts. Charts that animate, buttons that
  respond and anything interactive silently did nothing, and because the
  panel's own height reporting is an inline script too, every artifact appeared
  in a short fixed-height box whatever was in it. Development mode allows
  inline script, so nothing in ordinary use could show this. The frame is now
  served from its own scheme with its own policy, and the window keeps the
  stricter rule it gained in 1.5.3.
- A template whose footer reference came before its header reference was
  refused, with a message naming an internal identifier. Both orderings are
  ordinary Word output.
- A template using absolute relationship targets — legal, and what several
  tools write — had its header and footer treated as missing, and its page
  furniture silently dropped.
- Attaching a `.dotx` or `.potx` said "not a readable text file". They are what
